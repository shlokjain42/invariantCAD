...
The five roles apply only to compatible, parallel-plane, hole-free ruled solids whose ordered curves have the same kind, orientation, and authored curve phase across every section. Cyclically rotating an otherwise equivalent loop changes that authored correspondence and is rejected instead of allowing OCCT to choose a different pairing. If construction of any semantic seed fails, a side or lateral correspondence is not unique, a seed cannot be mapped uniquely into the result, or the expected role inventory is incomplete, the snapshot is marked partial rather than treating an incomplete map as authoritative. Sweep topology remains broadly attributed to its feature but has no cap/side roles or sketch-source mapping.
...
Legacy `DesignDocumentV1` remains parseable, cloneable, evaluable, and hash-stable, but cannot contain persistent selectors. `DesignDocumentV2` adds the optional document-owned `topologyReferences` registry and persistent selector atom while retaining the pre-loft closed role vocabulary. `DesignDocumentV3` adds only the five serialized loft roles, including those roles inside stored reference lineage and adjacency evidence; the current builder emits v3. Registry entries bind one topology kind and one exact solid-node target to one or more protocol/fingerprint variants. Registry data is semantic: normalized evidence and canonical variant order participate in serialization and hashing. Parsing, cloning, stringifying, hashing, and direct evaluation preserve v1 and v2 documents without silently rewriting them; explicit migration validates and upgrades v1 or v2 to v3, and is idempotent for v3. Migration never recaptures stored topology evidence, so a descriptor-`@2` variant remains `@2` until an application explicitly captures a compatible `@3` variant.
...
- Protocol-v1 detached face/edge capture and fail-closed resolution, plus Document-v2/v3 persistent selector atoms with exact target and fingerprint binding
...
| Persistent face/edge references | Protocol-v1 capture/resolution plus Document-v2/v3 registries and selector atoms, with exact target/fingerprint binding and no invented identity for symmetric topology | Broader naming across feature families and topology kinds |
...
| Semantic face/edge selectors | Origin/geometry/adjacency and Document-v2/v3 persistent-reference queries; v3 adds bounded ruled-loft roles to the frozen pre-loft v1/v2 vocabulary | Remaining feature-family roles and torture corpus |
...
The current authoring API emits `DesignDocumentV3`. `parseDocument`, `parseDocumentValue`, `stringifyDocument`, `cloneDocument`, and `hashDocument` preserve a supplied v1, v2, or v3 document. `migrateDocument` validates and upgrades v1 or v2 to v3 and is idempotent for v3. V1 cannot contain a topology-reference registry or persistent selector atom; v2 can, but both v1 and v2 retain the pre-loft topology-role vocabulary. The document schema version is independent of the npm package version, the OCCT topology-descriptor fingerprint, and persistent-reference protocol v1.
...
TypeScript builders
        │
        ▼
immutable DesignDocument v3 ──► validation / canonical JSON / hashing
    (v1/v2 remain readable and migratable)
        │

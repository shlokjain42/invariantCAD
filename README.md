...
The fingerprint is a semantic compatibility declaration for that kernel's topology descriptors, including any runtime or modeling-tolerance choices the kernel considers material. It is not a hash or cryptographic attestation of the native runtime bytes. Capture and resolution validate protocol v1 and require the fingerprint string to match exactly before considering a candidate. Applications should pass the capability advertised by the kernel that produced each snapshot; a missing declaration means that kernel has not promised compatibility with this protocol. The known stock runtime and recognized owned facades through ABI 0.4 use `invariantcad-topology-descriptor@4`, which adds bounded-sweep anchors to descriptor `@3`. Recognized owned facades ABI 0.5 and later use descriptor `@5`, which adds the two exact generated edge→face treatment roles. Each new descriptor is intentionally incompatible with its predecessor because authoritative semantic anchors change capture and resolution; persistent-reference protocol remains version 1. A stored `@4` variant therefore does not resolve against an `@5` declaration unless the entry is explicitly recaptured or supplemented with a matching `@5` variant. `createOcctKernel()` advertises the declaration for its known default stock runtime and for a recognized owned facade. Supplying an explicit `wasm` or an unknown custom `moduleFactory` suppresses it unless facade probing recognizes the matched owned runtime.

The executable [persistent-topology torture corpus](docs/persistent-topology-torture.md) covers both the stock descriptor `@4` floor and an explicit owned facade ABI 0.6 descriptor-`@5` exact-evolution matrix. Inherited Boolean, fillet/chamfer, shell, and draft items survive relevant parameter changes with fresh keys. The exact generated fillet/chamfer face resolves semantically through its proved class role across amount changes and drives downstream role and persistent selectors; a two-contour case fails capture as class-level ambiguity. Generated shell/offset faces and unnamed treatment edges remain geometry-only and may fail key-free when their evidence changes. The corpus never promotes surface resemblance or enumeration into persistent identity.
...
Legacy `DesignDocumentV1` remains parseable, cloneable, directly evaluable, and hash-stable, but cannot contain persistent selectors. `DesignDocumentV2` adds the optional document-owned `topologyReferences` registry and persistent selector atom while retaining the pre-loft closed role vocabulary. `DesignDocumentV3` adds only the five serialized loft roles, and `DesignDocumentV4` adds only the six bounded-sweep roles, including those same closed locations in stored lineage and adjacency evidence. `DesignDocumentV5` adds only `fillet.face.blend` and `chamfer.face.bevel`, and the current builder emits v5. Registry entries bind one topology kind and one exact solid-node target to one or more protocol/fingerprint variants. Registry data is semantic: normalized evidence and canonical variant order participate in serialization and hashing. Parsing, cloning, stringifying, hashing, validation, and direct evaluation preserve supplied v1–v4 documents without silently upgrading them. `migrateDocument` validates and upgrades v1–v4 to v5 and is idempotent for v5. Migration never captures, relabels, or rewrites stored descriptor evidence: a descriptor-`@4` fingerprint and its evidence remain exactly `@4` after document migration until an application explicitly captures or supplies a compatible `@5` variant.

These are independent version axes. Document v5 versions the serialized JSON grammar and closed role vocabulary. OCCT descriptor `@4` remains the stock/older-owned semantic declaration, while descriptor `@5` is conditional on recognized owned ABI 0.5+ exact edge-treatment evolution. Persistent-reference protocol v1 versions the detached evidence and matching envelope and remains unchanged; facade ABI remains 0.6.
...
- Protocol-v1 detached face/edge capture and fail-closed resolution, plus Document-v2–v5 persistent selector atoms with exact target and fingerprint binding
...
| Persistent face/edge references | Protocol-v1 capture/resolution plus versioned aggregate explanations and Document-v2–v5 registries/selector atoms, with exact target/fingerprint binding, a published initial torture corpus, and no invented identity for symmetric topology | Broader naming across feature families and topology kinds |
...
| Semantic face/edge selectors | Origin/geometry/adjacency and Document-v2–v5 persistent-reference queries; v3 adds bounded ruled-loft roles, v4 adds bounded-sweep roles, and v5 adds exact generated fillet/chamfer face roles while every earlier grammar stays frozen | Remaining feature-family roles and an expanded torture corpus |
...
The current authoring API emits `DesignDocumentV5`. `parseDocument`, `parseDocumentValue`, `stringifyDocument`, `cloneDocument`, `hashDocument`, validation, and evaluation preserve a supplied v1, v2, v3, v4, or v5 document. `migrateDocument` validates and upgrades v1–v4 to v5 and is idempotent for v5. V1 cannot contain a topology-reference registry or persistent selector atom; v2 can but retains the pre-loft role vocabulary; v3 adds loft roles but rejects sweep and edge-treatment roles; v4 adds the six sweep roles but rejects `fillet.face.blend` and `chamfer.face.bevel`; v5 adds exactly those two face roles. V1–v4 remain frozen and directly evaluable. Migration never rewrites stored descriptor fingerprints, lineage, geometry, or adjacency evidence. The document schema version is independent of the npm package version, conditional OCCT topology-descriptor `@4`/`@5`, facade ABI 0.6, and persistent-reference protocol v1.
...
TypeScript builders
        │
        ▼
immutable DesignDocument v5 ──► validation / canonical JSON / hashing
  (v1/v2/v3/v4 stay frozen, readable, directly evaluable, and migratable)
        │

...
These are four independent version axes. Document v6 versions the serialized JSON authoring grammar. Topology-signature protocol v2 versions current detached evidence and matching, while protocol v1 remains supported unchanged. OCCT descriptor `@6` is the primary v2 semantic declaration, with exact v1 descriptor `@4` or `@5` exposed as a compatibility profile according to the runtime. The owned facade ABI remains 0.6, and its separate `exactIndexedTopologyEvolution` protocol remains version 1.

...
Public semantic lineage is intentionally stricter than causal history. Only `PRESERVED` and `MODIFIED` identity predecessors inherit earlier lineage, roles, and sketch sources. A `GENERATED` link never copies a source role onto a new subshape; a result with only generated causes or a source-less `CREATED` record is recorded only as `createdBy(currentFeature)`. An identity successor keeps its proven earlier lineage and gains `modifiedBy(currentFeature)` only when an identity link is modified. The rule applies equally to ABI 0.4 Booleans and ABI 0.5 fillets/chamfers. These native indices and all public topology keys remain evaluation-scoped plumbing, never persistent document identity.
...
- Exact constant-radius edge fillets through the OCCT backend, with complete face/edge/vertex evolution through owned facade ABI 0.5
- Exact constant equal-distance edge chamfers through the OCCT backend, with complete face/edge/vertex evolution through owned facade ABI 0.5
...
- Exact topology enumeration, geometry/adjacency descriptors, selected-edge fillets/chamfers, face-selected shells, whole-solid offsets, owned-facade atomic draft, and owned-facade exact multi-input Boolean and edge-treatment evolution through OpenCascade WebAssembly
...
| Fillet/chamfer topology evolution | Complete face/edge/vertex graph with explicitly supplied owned OCCT facade ABI 0.5; partial on stock and owned ABI 0.2–0.4 | Persistent cross-evaluation naming |
| Loft | OCCT ordered ruled-solid mode with matched hole-free sections | Smooth, guided, and open modes |
| Sweep | OCCT open-polyline, one-edge circular-arc, and certified ordered line/arc composite solid modes with corrected-Frenet transport; owned facade ABI 0.5 retains the certified major multi-arc and eccentric-profile refinements introduced by ABI 0.3 | Bézier, B-spline, helix, guided, and variable-section modes |
...
The CLI selects stock OCCT automatically for `.step` and `.brep` destinations. Use `--kernel manifold|occt` to select a backend explicitly. The current CLI does not inject a custom module factory, so document draft evaluation, exact Boolean and fillet/chamfer topology evolution, and owned-facade-only composite refinements require programmatic initialization with the matched pair. Boolean and fillet/chamfer geometry itself remains available through supported stock paths with partial history.
...
That form still pairs the supplied binary with the stock `occt-wasm` JavaScript glue and therefore does not enable draft or exact Boolean or fillet/chamfer topology evolution. Load those guarantees with the generated JavaScript factory from the owned-facade build. A factory may locate its matched sibling WASM itself; pass `wasm` when the application or bundler needs an explicit binary URL:
...
`.artifacts/occt-facade-bundle/invariantcad-occt-facade-0.5.0/runtime/`.
Here `0.5.0` is the facade ABI/bundle version, not the npm package version.
...
in a fresh temporary consumer, and checks the owned ABI 0.5 capability surface
alongside direct/document-evaluated draft, exact Boolean and fillet/chamfer evolution, and major

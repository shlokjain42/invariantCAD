...
try {
  assert.deepEqual(kernel.capabilities.topology?.signatures, {
    protocolVersion: 2,
    fingerprint:
      "invariantcad-topology-descriptor@6;occt-wasm@3.8.0;runtime=invariantcad-facade@0.9.0+occt-wasm.3.8.0;modelingTolerance=1e-7",
  });
  assert.deepEqual(kernel.capabilities.topology?.signatureProfiles, [
    {
      protocolVersion: 1,
      fingerprint:
        "invariantcad-topology-descriptor@5;occt-wasm@3.8.0;runtime=invariantcad-facade@0.9.0+occt-wasm.3.8.0;modelingTolerance=1e-7",
    },
  ]);

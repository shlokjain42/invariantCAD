...
try {
  ...
  await writeFile(
    ...
    [
      ...
      'import { CIRCULAR_ARC_PATH_MIN_POINT_SINE, COMPOSITE_PATH_MAX_JUNCTION_SINE, DEFAULT_TOPOLOGY_SIGNATURE_LIMITS, EDGE_TOPOLOGY_ROLES, EDGE_TOPOLOGY_ROLES_V4, EDGE_TOPOLOGY_ROLES_V5, FACE_TOPOLOGY_ROLES, FACE_TOPOLOGY_ROLES_V4, FACE_TOPOLOGY_ROLES_V5, OFFSET_DIRECTIONS, OFFSET_JOIN_SEMANTICS, SHELL_DIRECTIONS, SHELL_JOIN_SEMANTICS, SWEEP_FRAMES, SWEEP_TRANSITIONS, TOPOLOGY_REFERENCE_EXPLANATION_VERSION, TOPOLOGY_ROLES, TOPOLOGY_ROLE_RULES, TOPOLOGY_ROLES_V4, TOPOLOGY_ROLES_V5, TOPOLOGY_SELECTION_EXPLANATION_VERSION, TOPOLOGY_SIGNATURE_PROTOCOL_VERSION, angleVec3, captureTopologyReference, createEvaluator, createTopologyReferenceResolutionSession, deg, design, explainTopologyReference, explainTopologySelection, kernelSupports, kgPerCubicMeter, mm, momentOfInertiaAboutAxis, plane, principalInertia, principalRadiiOfGyration, resolveTopologyReference, resolveTopologySelection, scalarVec3, stringifyDocument, tf, topology, vec3, worldRadiiOfGyration } from "invariantcad";',
      'import { DEFAULT_DESIGN_DOCUMENT_LIMITS, DOCUMENT_SCHEMA_V1, DOCUMENT_SCHEMA_V2, DOCUMENT_SCHEMA_V3, DOCUMENT_SCHEMA_V4, DOCUMENT_SCHEMA_V5, DOCUMENT_VERSION, DOCUMENT_VERSION_V1, DOCUMENT_VERSION_V2, DOCUMENT_VERSION_V3, DOCUMENT_VERSION_V4, DOCUMENT_VERSION_V5, DesignDocumentV1Schema, DesignDocumentV2Schema, DesignDocumentV3Schema, DesignDocumentV4Schema, DesignDocumentV5Schema, NodeSchema, NodeV1Schema, NodeV2Schema, NodeV3Schema, NodeV4Schema, NodeV5Schema, PersistentTopologyReferenceSchema, PersistentTopologyReferenceV4Schema, PersistentTopologyReferenceV5Schema, TopologyQuerySchema, TopologyQueryV1Schema, TopologyQueryV2Schema, TopologyQueryV3Schema, TopologyQueryV4Schema, TopologyQueryV5Schema, TopologyReferenceEntrySchema, TopologyReferenceEntryV4Schema, TopologyReferenceEntryV5Schema, TopologySelectionSchema, TopologySelectionV4Schema, TopologySelectionV5Schema, migrateDocument, parseDocument } from "invariantcad";',
      ...
      'const missingTopologyExplanation = explainTopologyReference(capturedTopologyReference.value, { history: "complete", faces: [], edges: [] }, { capabilities: topologySignatureCapabilities, limits: topologySignatureLimits });',
      ...
      'const ambiguousTopologySnapshot = { history: "complete", faces: [{ ...currentTopologySnapshot.faces[0], key: "ambiguous-first" }, { ...currentTopologySnapshot.faces[0], key: "ambiguous-second" }], edges: [] };',
      ...
      'const emptyTopologySnapshot = { history: "complete", faces: [], edges: [] };',
      ...
      'const malformedOrdinarySnapshotExplanation = explainTopologySelection(ordinaryFaceSelection, { history: "complete", faces: [currentTopologySnapshot.faces[0], currentTopologySnapshot.faces[0]], edges: [] }, ordinarySelectionContext);',
      ...
      'if (DOCUMENT_VERSION !== DOCUMENT_VERSION_V5 || DOCUMENT_SCHEMA_V4 !== "https://invariantcad.dev/schema/document/v4" || DOCUMENT_SCHEMA_V5 !== "https://invariantcad.dev/schema/document/v5" || !Object.isFrozen(DEFAULT_DESIGN_DOCUMENT_LIMITS)) throw new Error("Document-v5 public constants were not packaged");',
      'if (NodeSchema !== NodeV5Schema || TopologyQuerySchema !== TopologyQueryV5Schema || TopologySelectionSchema !== TopologySelectionV5Schema || PersistentTopologyReferenceSchema !== PersistentTopologyReferenceV5Schema || TopologyReferenceEntrySchema !== TopologyReferenceEntryV5Schema) throw new Error("Packed current schema aliases do not point at Document v5");',
      'if (TOPOLOGY_ROLES !== TOPOLOGY_ROLES_V5 || FACE_TOPOLOGY_ROLES !== FACE_TOPOLOGY_ROLES_V5 || EDGE_TOPOLOGY_ROLES !== EDGE_TOPOLOGY_ROLES_V5) throw new Error("Packed current topology vocabularies do not point at Document v5");',
      'for (const schema of [NodeV1Schema, NodeV2Schema, NodeV3Schema, NodeV4Schema, NodeV5Schema]) if (schema.safeParse({ kind: "futureFeature" }).success) throw new Error("Packed versioned node schema accepted an unknown future kind");',
      ...
      'if (persistentDocument.version !== 5 || persistentDocument.topologyReferences?.["stored-face"] === undefined) throw new Error("Document-v5 topology registry was not packaged");',
      'if (!DesignDocumentV5Schema.safeParse(persistentDocument).success || !PersistentTopologyReferenceV5Schema.safeParse(capturedTopologyReference.value).success || !TopologyReferenceEntryV5Schema.safeParse(persistentDocument.topologyReferences["stored-face"]).success || !TopologySelectionV5Schema.safeParse(persistentDocument.nodes["persistent-shell"].openings).success) throw new Error("Packed Document-v5 schema family is unsound");',
      'const parsedPersistentDocument = parseDocument(stringifyDocument(persistentDocument));',
      'if (!parsedPersistentDocument.ok || parsedPersistentDocument.value.version !== 5) throw new Error("Packed Document-v5 round trip failed");',
      'const persistentAtom = { op: "persistentReference", reference: "stored-face" };',
      'if (TopologyQueryV1Schema.safeParse(persistentAtom).success || !TopologyQueryV2Schema.safeParse(persistentAtom).success || !TopologyQueryV3Schema.safeParse(persistentAtom).success || !TopologyQueryV4Schema.safeParse(persistentAtom).success || !TopologyQueryV5Schema.safeParse(persistentAtom).success) throw new Error("Packed versioned query schemas are unsound");',
      'const loftRoleAtom = { op: "origin", feature: "loft", relation: "created", role: "loft.face.side" };',
      'if (TopologyQueryV1Schema.safeParse(loftRoleAtom).success || TopologyQueryV2Schema.safeParse(loftRoleAtom).success || !TopologyQueryV3Schema.safeParse(loftRoleAtom).success || !TopologyQueryV4Schema.safeParse(loftRoleAtom).success || !TopologyQueryV5Schema.safeParse(loftRoleAtom).success) throw new Error("Packed Document-v3 loft role boundary is unsound");',
      ...
      'for (const atom of [filletRoleAtom, chamferRoleAtom]) if (TopologyQueryV1Schema.safeParse(atom).success || TopologyQueryV2Schema.safeParse(atom).success || TopologyQueryV3Schema.safeParse(atom).success || TopologyQueryV4Schema.safeParse(atom).success || !TopologyQueryV5Schema.safeParse(atom).success) throw new Error("Packed Document-v5 edge-treatment role boundary is unsound");',
      ...
      'const legacyReferenceEntry = persistentDocument.topologyReferences["stored-face"];',
      'const persistentV2 = DesignDocumentV2Schema.parse({ ...persistentDocument, schema: DOCUMENT_SCHEMA_V2, version: DOCUMENT_VERSION_V2, topologyReferences: { "stored-face": { ...legacyReferenceEntry, variants: legacyReferenceEntry.variants.map((variant) => ({ ...variant, kernelFingerprint: "invariantcad-topology-descriptor@2;legacy-package-runtime" })) } } });',
      'const persistentV3 = DesignDocumentV3Schema.parse({ ...persistentDocument, schema: DOCUMENT_SCHEMA_V3, version: DOCUMENT_VERSION_V3, topologyReferences: { "stored-face": { ...legacyReferenceEntry, variants: legacyReferenceEntry.variants.map((variant) => ({ ...variant, kernelFingerprint: "invariantcad-topology-descriptor@3;legacy-package-runtime" })) } } });',
      'const persistentV4 = DesignDocumentV4Schema.parse({ ...persistentDocument, schema: DOCUMENT_SCHEMA_V4, version: DOCUMENT_VERSION_V4, topologyReferences: { "stored-face": { ...legacyReferenceEntry, variants: legacyReferenceEntry.variants.map((variant) => ({ ...variant, kernelFingerprint: "invariantcad-topology-descriptor@4;legacy-package-runtime" })) } } });',
      'for (const legacy of [persistentV2, persistentV3, persistentV4]) { const migrated = migrateDocument(legacy); if (!migrated.ok || migrated.value.version !== 5 || JSON.stringify(migrated.value.topologyReferences) !== JSON.stringify(legacy.topologyReferences)) throw new Error("Packed legacy registry migration rewrote persistent evidence"); }',
      'const remigratedV5 = migrateDocument(persistentDocument);',
      'if (!remigratedV5.ok || remigratedV5.value.version !== 5 || stringifyDocument(remigratedV5.value) !== stringifyDocument(persistentDocument)) throw new Error("Packed Document-v5 migration was not canonically idempotent");',
      ...
      'if (!migratedDocument.ok || migratedDocument.value.version !== 5) throw new Error("Packed v1-to-v5 migration failed");',
      "",
      ...
      '  if (!exactKernel.capabilities.topology?.signatures?.fingerprint.startsWith("invariantcad-topology-descriptor@4;")) throw new Error("OCCT descriptor fingerprint v4 was not packaged");',
      ...
      '  const packagedCapture = captureTopologyReference(snapshot, "face", persistentExactFace.key, { capabilities: exactKernel.capabilities.topology.signatures, tolerance: topologySignatureTolerance });',
      ...
      'const packagedPersistentCad = design("package-v5-persistent-evaluation");',
      ...
      'if (!packagedPersistentParsed.ok || packagedPersistentParsed.value.version !== 5) throw new Error("Packed V5 persistent document round trip failed");',
      ...
      '  if (!evaluated.ok) throw new Error("Packed V5 persistent registry evaluation failed: " + JSON.stringify(evaluated.diagnostics));',
      '  try { const measured = evaluated.value.output("persistent-shell").measure(); assertNear(measured.volume, 45.072, "Packed V5 parameter-changed persistent shell volume", 1e-8); } finally { evaluated.value.dispose(); }',
      '} finally {',
  ...
  if (ownedFacadeRuntimeDirectory !== undefined) {
    await writeFile(
      ...
      [
        ...
        '  assert.ok(kernel.capabilities.topology?.signatures?.fingerprint.startsWith("invariantcad-topology-descriptor@5;"), "Installed owned facade did not advertise descriptor @5");',
  ...
  await writeFile(
    ...
    [
      'import { DEFAULT_TOPOLOGY_SIGNATURE_LIMITS, EDGE_TOPOLOGY_ROLES, FACE_TOPOLOGY_ROLES, OFFSET_DIRECTIONS, SHELL_DIRECTIONS, TOPOLOGY_REFERENCE_EXPLANATION_VERSION, TOPOLOGY_ROLE_RULES, TOPOLOGY_ROLES, TOPOLOGY_SELECTION_EXPLANATION_VERSION, TOPOLOGY_SIGNATURE_PROTOCOL_VERSION, angleVec3, captureTopologyReference, combinePhysicalMassProperties, createTopologyReferenceResolutionSession, deg, design, explainTopologyReference, explainTopologySelection, gramsPerCubicCentimeter, inertiaTensorAboutPoint, kgPerCubicMeter, kgPerCubicMillimeter, mm, momentOfInertiaAboutAxis, physicalMassProperties, plane, principalInertia, principalRadiiOfGyration, radiusOfGyrationAboutAxis, resolveTopologyReference, resolveTopologySelection, scalarVec3, tf, topology, vec3, worldRadiiOfGyration, type BillOfMaterials, type BillOfMaterialsItem, type ChamferNodeIR, type ChamferNodeIRV5, type CircularArcPathNodeIR, type CompositePathNodeIR, type CompositePathSegmentExpression, type ConfigurationBuilder, type ConfigurationId, type ConfigurationOptions, type DesignConfigurationIR, type DesignDocument, type DraftNodeIR, type DraftNodeIRV5, type EdgeTopologyRole, type EdgeTopologyRoleV5, type EvaluatedAssembly, type EvaluatedPart, type EvaluationOptions, type ExplainableTopologyReferenceResolutionSession, type FaceTopologyRole, type FaceTopologyRoleV5, type FilletNodeIR, type FilletNodeIRV5, type InertiaAxis, type InertiaPropertySource, type InertiaTensor, type KernelTopologyKey, type KernelTopologySignatureCapabilities, type KernelTopologySnapshot, type LoftNodeIR, type MassDensityExpression, type MassDensitySource, type MaterialDefinitionIR, type MaterialRef, type OffsetDirection, type OffsetNodeIR, type PathRef, type PersistentTopologyLineageV5, type PersistentTopologyReference, type PersistentTopologyReferenceV5, type PhysicalInertiaTensor, type PhysicalMassProperties, type PolylinePathNodeIR, type PrincipalAxes, type PrincipalAxisStatus, type PrincipalInertiaDegeneracy, type PrincipalInertiaOptions, type PrincipalInertiaResult, type ProfileRef, type ResolvedCompositePath, type ResolvedTopologyReference, type ShellDirection, type ShellNodeIR, type ShellNodeIRV5, type SolidRef, type SweepNodeIR, type TopologyMatchTolerance, type TopologyNeighborSignatureV5, type TopologyOriginOptions, type TopologyReferenceEntryIRV5, type TopologyReferenceResolutionExplanation, type TopologyReferenceResolutionExplanationBase, type TopologyReferenceStrategySummary, type TopologyRole, type TopologyRoleV5, type TopologySelection, type TopologySelectionResolutionExplanation, type TopologySelectionResolutionExplanationBase, type TopologySignatureLimits, type VolumetricMassProperties } from "invariantcad";',
      'import { DEFAULT_DESIGN_DOCUMENT_LIMITS, DOCUMENT_VERSION_V5, type DesignDocumentLimits, type DesignDocumentV5, type NodeIR, type NodeIRV1, type NodeIRV2, type NodeIRV3, type NodeIRV4, type NodeIRV5, type TopologyQueryIR, type TopologyQueryIRV1, type TopologyQueryIRV2, type TopologyQueryIRV3, type TopologyQueryIRV4, type TopologyQueryIRV5, type TopologyReferenceOptions, type TopologyReferenceRef, type TopologySelectionIR, type TopologySelectionIRV1, type TopologySelectionIRV2, type TopologySelectionIRV3, type TopologySelectionIRV4, type TopologySelectionIRV5 } from "invariantcad";',
      ...
      'const persistentTopologyReferenceV5: PersistentTopologyReferenceV5<"face"> = persistentTopologyReference;',
      ...
      'const topologyDocument: DesignDocumentV5 = topologyCad.build();',
      'if (topologyDocument.version !== DOCUMENT_VERSION_V5) throw new Error("Typed Document-v5 build failed");',
      'const topologyReferenceEntryV5: TopologyReferenceEntryIRV5 = topologyDocument.topologyReferences![topologyReference.id]!;',
      'type Equivalent<A, B> = [A] extends [B] ? [B] extends [A] ? true : false : false;',
      'const packagedV5NodeAliases: readonly [Equivalent<FilletNodeIRV5, FilletNodeIR>, Equivalent<ChamferNodeIRV5, ChamferNodeIR>, Equivalent<ShellNodeIRV5, ShellNodeIR>, Equivalent<DraftNodeIRV5, DraftNodeIR>] = [true, true, true, true];',
      ...
      'const currentTopologySelection: TopologySelectionIRV5<"face"> = topology.faces.persistentReference(topologyReference).select().ir;',
      ...
      'const currentTopologyNode: NodeIRV5 = topologyDocument.nodes[topologyTarget.node];',
      ...
      '  void [invalidLegacyNode, invalidLegacyTopologyQuery, invalidLegacyTopologySelection, invalidV2LoftTopologyQuery, invalidV2Node, invalidV2TopologySelection, invalidV3Node, invalidV3SweepTopologyQuery, invalidV3TopologySelection, invalidV4FilletTopologyQuery, invalidV4Node, invalidV4TopologySelection];',
      '}',

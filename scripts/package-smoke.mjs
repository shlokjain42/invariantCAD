...
try {
  ...
  await writeFile(
    ...
    [
      ...
      'import { CIRCULAR_ARC_PATH_MIN_POINT_SINE, COMPOSITE_PATH_MAX_JUNCTION_SINE, DEFAULT_TOPOLOGY_SIGNATURE_LIMITS, EDGE_TOPOLOGY_ROLES, EDGE_TOPOLOGY_ROLES_V4, FACE_TOPOLOGY_ROLES, FACE_TOPOLOGY_ROLES_V4, OFFSET_DIRECTIONS, OFFSET_JOIN_SEMANTICS, SHELL_DIRECTIONS, SHELL_JOIN_SEMANTICS, SWEEP_FRAMES, SWEEP_TRANSITIONS, TOPOLOGY_REFERENCE_EXPLANATION_VERSION, TOPOLOGY_ROLES, TOPOLOGY_ROLE_RULES, TOPOLOGY_ROLES_V4, TOPOLOGY_SELECTION_EXPLANATION_VERSION, TOPOLOGY_SIGNATURE_PROTOCOL_VERSION, angleVec3, captureTopologyReference, createEvaluator, createTopologyReferenceResolutionSession, deg, design, explainTopologyReference, explainTopologySelection, kernelSupports, kgPerCubicMeter, mm, momentOfInertiaAboutAxis, plane, principalInertia, principalRadiiOfGyration, resolveTopologyReference, resolveTopologySelection, scalarVec3, stringifyDocument, tf, topology, vec3, worldRadiiOfGyration } from "invariantcad";',
      'import { DEFAULT_DESIGN_DOCUMENT_LIMITS, DOCUMENT_SCHEMA_V1, DOCUMENT_SCHEMA_V2, DOCUMENT_SCHEMA_V3, DOCUMENT_SCHEMA_V4, DOCUMENT_VERSION, DOCUMENT_VERSION_V1, DOCUMENT_VERSION_V2, DOCUMENT_VERSION_V3, DOCUMENT_VERSION_V4, DesignDocumentV1Schema, DesignDocumentV2Schema, DesignDocumentV3Schema, DesignDocumentV4Schema, NodeSchema, NodeV1Schema, NodeV2Schema, NodeV3Schema, NodeV4Schema, PersistentTopologyReferenceSchema, PersistentTopologyReferenceV4Schema, TopologyQuerySchema, TopologyQueryV1Schema, TopologyQueryV2Schema, TopologyQueryV3Schema, TopologyQueryV4Schema, TopologyReferenceEntrySchema, TopologyReferenceEntryV4Schema, TopologySelectionSchema, TopologySelectionV4Schema, migrateDocument, parseDocument } from "invariantcad";',
      ...
      'if (DOCUMENT_VERSION !== DOCUMENT_VERSION_V4 || DOCUMENT_SCHEMA_V3 !== "https://invariantcad.dev/schema/document/v3" || DOCUMENT_SCHEMA_V4 !== "https://invariantcad.dev/schema/document/v4" || !Object.isFrozen(DEFAULT_DESIGN_DOCUMENT_LIMITS)) throw new Error("Document-v4 public constants were not packaged");',
      'if (NodeSchema !== NodeV4Schema || TopologyQuerySchema !== TopologyQueryV4Schema || TopologySelectionSchema !== TopologySelectionV4Schema || PersistentTopologyReferenceSchema !== PersistentTopologyReferenceV4Schema || TopologyReferenceEntrySchema !== TopologyReferenceEntryV4Schema) throw new Error("Packed current schema aliases do not point at Document v4");',
      'if (TOPOLOGY_ROLES !== TOPOLOGY_ROLES_V4 || FACE_TOPOLOGY_ROLES !== FACE_TOPOLOGY_ROLES_V4 || EDGE_TOPOLOGY_ROLES !== EDGE_TOPOLOGY_ROLES_V4) throw new Error("Packed current topology vocabularies do not point at Document v4");',
      'for (const schema of [NodeV1Schema, NodeV2Schema, NodeV3Schema, NodeV4Schema]) if (schema.safeParse({ kind: "futureFeature" }).success) throw new Error("Packed versioned node schema accepted an unknown future kind");',
      ...
      'if (persistentDocument.version !== 4 || persistentDocument.topologyReferences?.["stored-face"] === undefined) throw new Error("Document-v4 topology registry was not packaged");',
      'if (!DesignDocumentV4Schema.safeParse(persistentDocument).success || !PersistentTopologyReferenceV4Schema.safeParse(capturedTopologyReference.value).success || !TopologyReferenceEntryV4Schema.safeParse(persistentDocument.topologyReferences["stored-face"]).success || !TopologySelectionV4Schema.safeParse(persistentDocument.nodes["persistent-shell"].openings).success) throw new Error("Packed Document-v4 schema family is unsound");',
      'const parsedPersistentDocument = parseDocument(stringifyDocument(persistentDocument));',
      'if (!parsedPersistentDocument.ok || parsedPersistentDocument.value.version !== 4) throw new Error("Packed Document-v4 round trip failed");',
      'const persistentAtom = { op: "persistentReference", reference: "stored-face" };',
      'if (TopologyQueryV1Schema.safeParse(persistentAtom).success || !TopologyQueryV2Schema.safeParse(persistentAtom).success || !TopologyQueryV3Schema.safeParse(persistentAtom).success || !TopologyQueryV4Schema.safeParse(persistentAtom).success) throw new Error("Packed versioned query schemas are unsound");',
      'const loftRoleAtom = { op: "origin", feature: "loft", relation: "created", role: "loft.face.side" };',
      'if (TopologyQueryV1Schema.safeParse(loftRoleAtom).success || TopologyQueryV2Schema.safeParse(loftRoleAtom).success || !TopologyQueryV3Schema.safeParse(loftRoleAtom).success || !TopologyQueryV4Schema.safeParse(loftRoleAtom).success) throw new Error("Packed Document-v3 loft role boundary is unsound");',
      'const sweepRoleAtom = { op: "origin", feature: "sweep", relation: "created", role: "sweep.face.side" };',
      'if (TopologyQueryV1Schema.safeParse(sweepRoleAtom).success || TopologyQueryV2Schema.safeParse(sweepRoleAtom).success || TopologyQueryV3Schema.safeParse(sweepRoleAtom).success || !TopologyQueryV4Schema.safeParse(sweepRoleAtom).success) throw new Error("Packed Document-v4 sweep role boundary is unsound");',
      'if (!Object.isFrozen(TOPOLOGY_ROLES_V4) || !TOPOLOGY_ROLES_V4.includes("sweep.edge.lateral")) throw new Error("Packed Document-v4 topology vocabulary is unsound");',
      ...
      'for (const legacy of [persistentV2, persistentV3]) { const migrated = migrateDocument(legacy); if (!migrated.ok || migrated.value.version !== 4 || JSON.stringify(migrated.value.topologyReferences) !== JSON.stringify(legacy.topologyReferences)) throw new Error("Packed legacy registry migration rewrote persistent evidence"); }',
      ...
      'if (!migratedDocument.ok || migratedDocument.value.version !== 4) throw new Error("Packed v1-to-v4 migration failed");',
      "",
      ...
      'const packagedPersistentCad = design("package-v4-persistent-evaluation");',
      ...
      'if (!packagedPersistentParsed.ok || packagedPersistentParsed.value.version !== 4) throw new Error("Packed V4 persistent document round trip failed");',
      ...
      '  if (!evaluated.ok) throw new Error("Packed V4 persistent registry evaluation failed: " + JSON.stringify(evaluated.diagnostics));',
      '  try { const measured = evaluated.value.output("persistent-shell").measure(); assertNear(measured.volume, 45.072, "Packed V4 parameter-changed persistent shell volume", 1e-8); } finally { evaluated.value.dispose(); }',
      '} finally {',
  ...
  await writeFile(
    ...
    [
      ...
      'const topologyDocument: DesignDocumentV4 = topologyCad.build();',
      'if (topologyDocument.version !== DOCUMENT_VERSION_V4) throw new Error("Typed Document-v4 build failed");',
      ...
      'const currentTopologySelection: TopologySelectionIRV4<"face"> = topology.faces.persistentReference(topologyReference).select().ir;',
      ...
      'const currentTopologyNode: NodeIRV4 = topologyDocument.nodes[topologyTarget.node];',
      ...
      '  // @ts-expect-error A widened v4 selection cannot be assigned to the v1 grammar.',
      ...
      '  // @ts-expect-error A widened v4 selection cannot be assigned to the frozen v2 grammar.',
      ...
      '  // @ts-expect-error A widened v4 selection cannot be assigned to the frozen v3 grammar.',
      '  const invalidV3TopologySelection: TopologySelectionIRV3<"face"> = currentTopologySelection;',
      '  // @ts-expect-error A widened v4 node cannot be assigned to the v1 grammar.',
      '  const invalidLegacyNode: NodeIRV1 = currentTopologyNode;',
      '  // @ts-expect-error A widened v4 node cannot be assigned to the frozen v2 grammar.',
      '  const invalidV2Node: NodeIRV2 = currentTopologyNode;',
      '  // @ts-expect-error A widened v4 node cannot be assigned to the frozen v3 grammar.',
      '  const invalidV3Node: NodeIRV3 = currentTopologyNode;',
      '  void [invalidLegacyNode, invalidLegacyTopologyQuery, invalidLegacyTopologySelection, invalidV2LoftTopologyQuery, invalidV2Node, invalidV2TopologySelection, invalidV3Node, invalidV3SweepTopologyQuery, invalidV3TopologySelection];',
      '}',
      ...
      'void [currentNodeAlias, currentSweepEdgeRole, currentSweepFaceRole, currentSweepRole, currentSweepTopologyQuery, currentSweepTopologySelection, currentTopologyNode, currentTopologySelection, documentLimits, explainedTopologyKey, legacyTopologyQuery, loftTopologyQueryV3, ordinarySelectionExplanation, ordinarySelectionExplanationBase, ordinarySelectionKeys, ordinarySelectionMaximum, ordinarySelectionResolution, ordinaryTopologySelection, persistentTopologyQueryV2, persistentTopologyReference, resolvedTopologyReference, sweepTopologyQueryV4, topologyExplanation, topologyExplanationBase, topologyExplanationVersion, topologyReference, topologySelectionExplanationVersion, topologySession, topologySignatureLimits, topologyStrategySummary];',
      "",

...
try {
  ...
  await writeFile(
    ...
    [
      ...
      'if (persistentDocument.version !== 3 || persistentDocument.topologyReferences?.["stored-face"] === undefined) throw new Error("Document-v3 topology registry was not packaged");',
      'const parsedPersistentDocument = parseDocument(stringifyDocument(persistentDocument));',
      'if (!parsedPersistentDocument.ok || parsedPersistentDocument.value.version !== 3) throw new Error("Packed Document-v3 round trip failed");',
      'const persistentAtom = { op: "persistentReference", reference: "stored-face" };',
      'if (TopologyQueryV1Schema.safeParse(persistentAtom).success || !TopologyQueryV2Schema.safeParse(persistentAtom).success || !TopologyQueryV3Schema.safeParse(persistentAtom).success) throw new Error("Packed versioned query schemas are unsound");',
      'const loftRoleAtom = { op: "origin", feature: "loft", relation: "created", role: "loft.face.side" };',
      'if (TopologyQueryV1Schema.safeParse(loftRoleAtom).success || TopologyQueryV2Schema.safeParse(loftRoleAtom).success || !TopologyQueryV3Schema.safeParse(loftRoleAtom).success) throw new Error("Packed Document-v3 loft role boundary is unsound");',
      ...
      'if (!migratedDocument.ok || migratedDocument.value.version !== 3) throw new Error("Packed v1-to-v3 migration failed");',
      "",
      ...
      '  if (!exactKernel.capabilities.topology?.signatures?.fingerprint.startsWith("invariantcad-topology-descriptor@3;")) throw new Error("OCCT descriptor fingerprint v3 was not packaged");',
  ...
  await writeFile(
    ...
    [
      ...
      'import { DEFAULT_DESIGN_DOCUMENT_LIMITS, DOCUMENT_VERSION_V3, type DesignDocumentLimits, type DesignDocumentV3, type NodeIRV1, type NodeIRV2, type NodeIRV3, type TopologyQueryIRV1, type TopologyQueryIRV2, type TopologyQueryIRV3, type TopologyReferenceOptions, type TopologyReferenceRef, type TopologySelectionIRV1, type TopologySelectionIRV2, type TopologySelectionIRV3 } from "invariantcad";',
      ...
      'const topologyDocument: DesignDocumentV3 = topologyCad.build();',
      'if (topologyDocument.version !== DOCUMENT_VERSION_V3) throw new Error("Typed Document-v3 build failed");',
      ...
      'const currentTopologySelection: TopologySelectionIRV3<"face"> = topology.faces.persistentReference(topologyReference).select().ir;',
      'const currentTopologyNode: NodeIRV3 = topologyDocument.nodes[topologyTarget.node];',
      'if (false) {',
      ...
      '  // @ts-expect-error A widened v3 selection cannot be assigned to the v1 grammar.',
      ...
      '  // @ts-expect-error A widened v3 selection cannot be assigned to the frozen v2 grammar.',
      '  const invalidV2TopologySelection: TopologySelectionIRV2<"face"> = currentTopologySelection;',
      '  // @ts-expect-error A widened v3 node cannot be assigned to the v1 grammar.',
      '  const invalidLegacyNode: NodeIRV1 = currentTopologyNode;',
      '  // @ts-expect-error A widened v3 node cannot be assigned to the frozen v2 grammar.',
      '  const invalidV2Node: NodeIRV2 = currentTopologyNode;',
      '  void [invalidLegacyNode, invalidLegacyTopologyQuery, invalidLegacyTopologySelection, invalidV2LoftTopologyQuery, invalidV2Node, invalidV2TopologySelection];',
      '}',
      ...
      'void [currentTopologyNode, currentTopologySelection, documentLimits, legacyTopologyQuery, loftTopologyQueryV3, persistentTopologyQueryV2, persistentTopologyReference, resolvedTopologyReference, topologyReference, topologySignatureLimits];',
      "",

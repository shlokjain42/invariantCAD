...
try {
  ...
  await writeFile(
    ...
    [
      ...
      'import { DEFAULT_DESIGN_DOCUMENT_LIMITS, DOCUMENT_SCHEMA_V1, DOCUMENT_VERSION, DOCUMENT_VERSION_V1, DesignDocumentV1Schema, TopologyQueryV1Schema, TopologyQueryV2Schema, migrateDocument, parseDocument } from "invariantcad";',
      ...
      'if (DOCUMENT_VERSION !== 2 || !Object.isFrozen(DEFAULT_DESIGN_DOCUMENT_LIMITS)) throw new Error("Document-v2 public constants were not packaged");',
      ...
      'if (persistentDocument.version !== 2 || persistentDocument.topologyReferences?.["stored-face"] === undefined) throw new Error("Document-v2 topology registry was not packaged");',
      'const parsedPersistentDocument = parseDocument(stringifyDocument(persistentDocument));',
      'if (!parsedPersistentDocument.ok || parsedPersistentDocument.value.version !== 2) throw new Error("Packed Document-v2 round trip failed");',
      'const persistentAtom = { op: "persistentReference", reference: "stored-face" };',
      'if (TopologyQueryV1Schema.safeParse(persistentAtom).success || !TopologyQueryV2Schema.safeParse(persistentAtom).success) throw new Error("Packed versioned query schemas are unsound");',
      ...
      'if (!migratedDocument.ok || migratedDocument.value.version !== 2) throw new Error("Packed v1-to-v2 migration failed");',
      "",
  ...
  await writeFile(
    ...
    [
      ...
      'import { DEFAULT_DESIGN_DOCUMENT_LIMITS, DOCUMENT_VERSION_V2, type DesignDocumentLimits, type DesignDocumentV2, type NodeIRV1, type NodeIRV2, type TopologyQueryIRV1, type TopologyReferenceOptions, type TopologyReferenceRef, type TopologySelectionIRV1, type TopologySelectionIRV2 } from "invariantcad";',
      ...
      'const topologyDocument: DesignDocumentV2 = topologyCad.build();',
      'if (topologyDocument.version !== DOCUMENT_VERSION_V2) throw new Error("Typed Document-v2 build failed");',
      'const legacyTopologyQuery: TopologyQueryIRV1 = { op: "all" };',
      'const currentTopologySelection: TopologySelectionIRV2<"face"> = topology.faces.persistentReference(topologyReference).select().ir;',
      'const currentTopologyNode: NodeIRV2 = topologyDocument.nodes[topologyTarget.node];',
      'if (false) {',
      ...
      '  // @ts-expect-error A widened v2 selection cannot be assigned to the v1 grammar.',
      '  const invalidLegacyTopologySelection: TopologySelectionIRV1<"face"> = currentTopologySelection;',
      '  // @ts-expect-error A widened v2 node cannot be assigned to the v1 grammar.',
      '  const invalidLegacyNode: NodeIRV1 = currentTopologyNode;',
      '  void [invalidLegacyNode, invalidLegacyTopologyQuery, invalidLegacyTopologySelection];',
      '}',
      ...
      'void [currentTopologyNode, currentTopologySelection, documentLimits, legacyTopologyQuery, persistentTopologyReference, resolvedTopologyReference, topologyReference, topologySignatureLimits];',
      "",

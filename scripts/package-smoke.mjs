...
try {
  ...
  if (ownedFacadeRuntimeDirectory !== undefined) {
    await writeFile(
      ...
      [
        ...
        '  assert.equal(module.invariantcadFacadeVersion(), "invariantcad-facade@0.5.0+occt-wasm.3.7.0");',
        ...
        '  for (const feature of ["draft", "boolean", "fillet", "chamfer"]) if (!kernel.capabilities.features.includes(feature)) throw new Error(`Installed kernel did not advertise owned exact ${feature} topology`);',
        "  assert.deepEqual(kernel.capabilities.exactIndexedTopologyEvolution, { protocolVersion: 1, features: [\"draft\", \"boolean\", \"fillet\", \"chamfer\"] });",
        ...
        '  if (kernel.draft === undefined || kernel.boolean === undefined || kernel.fillet === undefined || kernel.chamfer === undefined || kernel.topology === undefined) throw new Error("Installed owned facade lacks exact topology support");',
        "",
        "  let directBox;",
        "  let directDraft;",
        "  try {",
  ...
  await writeFile(
    ...
    [
      ...
      'import { DEFAULT_OCCT_EXACT_BOOLEAN_HISTORY_RECORD_LIMIT, DEFAULT_OCCT_EXACT_EDGE_TREATMENT_HISTORY_RECORD_LIMIT, createOcctKernel, type OcctKernelOptions, type OcctModuleFactory, type OcctModuleOptions } from "invariantcad/kernels/occt";',
      "",
      ...
      "const options: OcctKernelOptions = { moduleFactory, maxExactBooleanHistoryRecords: DEFAULT_OCCT_EXACT_BOOLEAN_HISTORY_RECORD_LIMIT, maxExactEdgeTreatmentHistoryRecords: DEFAULT_OCCT_EXACT_EDGE_TREATMENT_HISTORY_RECORD_LIMIT };",

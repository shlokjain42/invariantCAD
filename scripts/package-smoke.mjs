...
try {
  ...
  await writeFile(
    ...
    [
      ...
      'const packagedPersistentBox = packagedPersistentCad.box("exact-box", { size: vec3(mm(2), mm(3), mm(4)) });',
      'const packagedPersistentOpening = packagedPersistentCad.topologyReference("x-min-opening", packagedPersistentBox, { topology: "face", variants: [packagedOcctFaceReference] });',
      ...
      'try {',
      '  const evaluated = await packagedPersistentEvaluator.evaluate(packagedPersistentParsed.value);',
      '  if (!evaluated.ok) throw new Error("Packed V4 persistent registry evaluation failed: " + JSON.stringify(evaluated.diagnostics));',
      '  try { const measured = evaluated.value.output("persistent-shell").measure(); if (!(measured.volume > 0) || !(measured.volume < 24)) throw new Error("Packed V4 persistent selector produced an invalid shell"); } finally { evaluated.value.dispose(); }',
      '} finally {',

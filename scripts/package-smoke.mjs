...
try {
  ...
  if (ownedFacadeRuntimeDirectory !== undefined) {
    await writeFile(
      ...
      [
        ...
        '  assert.equal(module.invariantcadFacadeVersion(), "invariantcad-facade@0.4.0+occt-wasm.3.7.0");',
        ...
        '  if (!kernel.capabilities.features.includes("draft") || !kernel.capabilities.features.includes("boolean")) throw new Error("Installed kernel did not advertise owned exact topology features");',
        "  assert.deepEqual(kernel.capabilities.exactIndexedTopologyEvolution, { protocolVersion: 1, features: [\"draft\", \"boolean\"] });",
        ...
        '  if (kernel.draft === undefined || kernel.boolean === undefined || kernel.topology === undefined) throw new Error("Installed owned facade lacks exact topology support");',
        "",
        "  let directBox;",
        "  let directDraft;",
        "  try {",
  ...
  await writeFile(
    ...
    [
      ...
      'import { DEFAULT_OCCT_EXACT_BOOLEAN_HISTORY_RECORD_LIMIT, createOcctKernel, type OcctKernelOptions, type OcctModuleFactory, type OcctModuleOptions } from "invariantcad/kernels/occt";',
      "",

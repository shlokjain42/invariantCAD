...
exports STEP. It also exercises the repository-private OCCT artifact candidate
inside disposable stock-runtime module workers; that test does not advertise a
public codec.
...
runtime. It is not live/peak-memory measurement, a real OCCT-trap injection,
general runtime/build attestation, trusted-host isolation, durable subshape
identity, a cross-platform golden matrix, or evaluator/cache integration.
...
producer and consumer child processes, verifies release-input JS/WASM
size/SHA-256 before using copied bytes as execution inputs, covers `SIGKILL`
deadline/abort and injected-trap recovery, and explicitly reports
...
the public kernels and the stock-runtime module-worker scenario described
above. Other evergreen browsers are plausible but are not claimed as
release-gated until executable coverage is added. The private candidate remains
unexported, `shapeArtifacts` remains absent, and these tests confer no

...
Node uses a process-global module hook without temporary executable files and
requires worker permission under the Node Permission Model. Browser loading
uses a revocable Blob module URL and needs a compatible `blob:` content
security policy.

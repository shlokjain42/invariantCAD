...
`examples/docs/`. Edit the canonical source rather than content between
`docs-example` markers, then run `pnpm docs:generate`. `pnpm docs:check`
verifies exact synchronization and source-mapped public imports without
starting native runtimes; run `pnpm test:docs-examples` for the separate
Manifold and OCCT execution gate. Unmarked illustrative fences remain
hand-maintained.

...
In Node.js, the loader installs one process-global `node:module.register()` hook
and leaves the evaluated module cache in place for the process lifetime; it
does not create a temporary executable file. Node's Permission Model must allow
the hook worker. In browsers, the loader imports a unique Blob module URL and
requires `blob:` in the applicable `script-src` policy. A Worker or child
process remains the hard reclamation boundary.

...
Evaluation accepts a caller resolver, `maxResourceBytes`, document limits, and
an abort signal. It verifies the authored byte length and SHA-256 digest before
native parsing, requires the strong document-body import capability for the
exact format/unit pair, and returns structured diagnostics for missing
resolution, integrity mismatch, limits, cancellation, or kernel failure. Read
the [verified single-body workflow](/interchange/import-export#verified-single-body-import)

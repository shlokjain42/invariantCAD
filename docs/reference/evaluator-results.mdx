...
direct graph admitted by the staged import, body-set, part, and flat
local-assembly evaluators, plus the nodes admitted by the datum resolver,
without hand-writing v7 IR. It admits typed length, mass-density, and scalar
parameters; named parameter, part-material, and root-instance-suppression
...
set; flat local-part assemblies; and datum points, axes, planes, and coordinate
systems. Outputs may directly reference an imported body, body set, part, or
staged local assembly; datums are selected separately by node ID and are never
design outputs. The built `DesignDocumentV7` is detached, deeply frozen, and
accepted by strict v7 parsing and semantic validation; `build({ limits })` can
apply partial `DesignDocumentLimits` overrides and raises a `CadError` rather
than returning an invalid document. The facade is not a root or
package-subpath export and does not change the public v6 builder or evaluator
types.
...
This first evaluator slice admits only active, direct local-part instances.
Each occurrence keeps its authored stable ID as a one-segment path, the local
part node, effective configuration identity, effective material and density
provenance, and resolved affine placement. The root configuration determines
definition-scoped suppression for the selected assembly. An instance then
selects its part context independently:

- `inherit` uses the root assembly's selected configuration;
- `base` uses the base document context; and
- `named` uses the referenced document-owned named configuration.

The occurrence ID, one-segment path, effective configuration ID, and resolved
transform are detached, frozen snapshots. The embedded `part` is different: it
is an owner-bound `EvaluatedPartV7` shared for the applicable part/configuration
context, and its operations fail after the containing evaluated design is
disposed.
...
an effectively suppressed unsupported component in hand-authored v7 data is
inert; an active nested local assembly or external component produces a
structured unsupported diagnostic before resource resolution or kernel work.
The evaluator performs no external-document read or recursive assembly
expansion.
...
overrides, authored instances scanned, active occurrences, placement
operations, effective part contexts, part-body memberships, distinct acquired
solids, and resolved materials. It also applies caller-selected document and
resource-resolution limits and checks cancellation throughout option capture,
document admission, planning, resource resolution, kernel acquisition, and
the final success boundary. These limits do not bound mesh buffers created by
later result methods; hostile or very large tessellation still belongs in a
...
The staged geometry slices still exclude nested local assemblies, external
component evaluation, datum-backed sketches, healing, location I/O,

...
import, body-set, part, and nested local-assembly evaluators, plus the nodes
...
generic primitive, Boolean, or transformed-solid outputs remain unsupported.
...
supported Boolean/transformed single-solid or multibody geometry as a direct
staged part, including commitment-verified imported leaves.
...
solid-graph nodes, Boolean/transform dependency links, transform operations,
and resolved materials. `maxSolidGraphNodes`, `maxSolidDependencyLinks`, and
...
and suppressed subtrees are pruned before the counters advance. Boolean target
and tool edges, including repeated references, and transform input edges all
contribute to the dependency count. The explicit
...
Boolean or transform inputs that are body sets/parts/assemblies, other
body-consuming operations, and general solid graphs beyond
primitive/import/Boolean/transform DAGs. Generic primitive, Boolean, or
transformed-solid outputs remain unsupported; a solid result must be retained
by a body set or part. The slices add no mates, joints,
degrees-of-freedom solving, motion, contact, interference, collision,

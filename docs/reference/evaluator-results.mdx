...
## Repository-only staged Document v7 imports, body sets, and parts

The repository-only `stagedBodySetDesignV7(...)` facade can now author the
direct graph admitted by the staged import, body-set, and part evaluators
without hand-writing v7 IR. It admits typed length and mass-density parameters,
named parameter and part-material configurations, document-owned materials,
boxes, cylinders, spheres, content commitments, imported bodies, body sets,
and parts over one direct leaf or body set. Outputs may directly reference an
imported body, body set, or part. The built `DesignDocumentV7` is detached,
deeply frozen, and accepted by strict v7 parsing and semantic validation;
`build({ limits })` can apply partial `DesignDocumentLimits` overrides and
raises a `CadError` rather than returning an invalid document. The facade is
not a root or package-subpath export and does not change the public v6 builder
or evaluator types.
...
The staged part slice still excludes assemblies, external occurrences, datums,
healing, location I/O, body-consuming operations, and general dependency
graphs. It does not widen the package-root `EvaluatedOutput` union, public

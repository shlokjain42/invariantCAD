...
direct graph admitted by the staged import and body-set evaluators without
...
outputs are limited to a direct imported-body or direct body-set reference; it
does not author parts. The built `DesignDocumentV7` is detached, deeply frozen,
and accepted by strict v7 parsing and semantic validation; `build({ limits })`
can apply partial `DesignDocumentLimits` overrides and raises a `CadError`
rather than returning an invalid document. The facade is not a root or
...
body, or a body set admitted by the staged body-set evaluator. This does not
add part authoring: callers currently reach the slice through strictly parsed
v7 documents, and `stagedBodySetDesignV7(...)` still cannot construct a part.
Its source-only contract types are `EvaluatePartOutputsV7Options`,

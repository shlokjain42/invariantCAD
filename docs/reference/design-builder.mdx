...
This is a deliberately narrow facade for authoring the direct graph, acyclic
nested local assemblies, and datum nodes admitted by the four staged v7
geometry evaluators and the datum resolver. Its implementation lives in
...
| `configuration(...)` | Configuration ID | Its callback admits typed length/density/scalar `.parameter(...)` overrides, `.partMaterial(part, material)` substitution, and `.instanceSuppressed(assembly, instanceId, suppressed?)` |
...
The recently admitted part and material source signatures are:
...
Length, density, and scalar parameters may be specialized by a named
...
leaf acquisition within each configuration batch. Authoring does not create a
persistent or cross-run cache commitment.
...
resource ceilings. Hand-authored cyclic dependencies fail strict document
admission, and evaluation also rejects a recursive path rather than looping.
...
external occurrences, cyclic assembly graphs, datum-backed sketches,
solid transform nodes, Booleans, body-consuming features, per-body materials,
or other general dependency graphs. It also does not add datum design outputs,
CLI support, location I/O, reader inference, healing, a primary-body rule,

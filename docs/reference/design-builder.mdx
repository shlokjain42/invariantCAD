...
This is a deliberately narrow facade for authoring the direct graph and datum
nodes already admitted by the three staged v7 geometry evaluators and the datum
...
imported-body, body-set, part, and datum records. Its complete admitted surface
is:
...
| `configuration(...)` | Configuration ID | Its callback admits typed length/density/scalar `.parameter(...)` overrides and `.partMaterial(part, material)` substitution |
...
dispatching through mutable handle methods. Datum authoring returns
...
configuration, material, part, resource, import, unit-policy, datum, or
body-member option record contains an accessor or unknown field. Those inputs
must be plain own-data records; accessors are not invoked and unsupported
...
assemblies, external occurrences, datum-backed sketches, transforms, Booleans,
body-consuming features, per-body materials, or other general dependency
graphs. It also does not add datum design outputs, CLI support, location I/O,
reader inference, healing, a primary-body rule, cross-body topology, aggregate
geometric measurement, or exact aggregate STEP/BREP export. Part-level STL/OBJ
export is an aggregate tessellation view and is explicitly approximate/lossy;
exact native export and topology remain per-solid or per-body capabilities.
Those omissions are product boundaries, not implicit fallbacks.

...
primitive/import/transform solid DAGs, acyclic nested local assemblies, and
datum nodes admitted by the four staged v7 geometry evaluators and the datum
resolver. Its implementation lives in
...
| `bodySet(id, members)` | `StagedBodySetRefV7` | Dense non-empty ordered memberships whose roots are owned primitive/import/transform solid DAGs |
...
primitive, imported-body, or transformed-solid DAG root, or one owned body
set. It cannot wrap another part, assembly, or body set as a transform input.
...
### Staged solid transform DAGs

`StagedSolidRefV7` is the owner-bound handle shared by direct primitive leaves,
imported-body leaves, and transform results. `transform(...)` requires at least
one operation and captures its dense ordered operation list as plain own data.
...
body-set membership, or a part's single-solid geometry.

The staged evaluator plans the selected closure iteratively, deduplicates
shared nodes, and executes every dependency before its consuming transform.
`maxDistinctSolids` counts primitive and imported leaves.
`maxSolidGraphNodes` counts all distinct admitted solid nodes,
`maxSolidDependencyLinks` counts transform input edges, and
...
per-solid topology/native-export capabilities. Manifold evaluates the native
primitive/transform graph as an approximate mesh. The evaluator never retries
an exact failure on Manifold or silently converts an exact graph to a mesh
graph. Any imported leaf, transformed or not, additionally requires the strong
exact B-Rep single-solid document-import capability, so it cannot fall back to
Manifold's weak or absent import path.

This is not general shape algebra. A transform input must be an owned
`StagedSolidRefV7` created by a primitive, imported body, or prior transform;
the facade does not author Boolean solids, and a body set, part, or assembly
cannot be transformed. A generic or transformed solid also cannot be
published directly by `output(...)`: it must be retained by a body set or
part. The one exception to generic-solid output is the narrower direct
`StagedImportedBodyRefV7` output already admitted by the import evaluator.
...
resource ceilings. Solid-graph nodes, transform input edges, and transform
...
external occurrences, cyclic assembly graphs, datum-backed sketches, Boolean
nodes, transforms over body sets/parts/assemblies, other body-consuming
features, per-body materials, or general solid graphs beyond
primitive/import/transform DAGs. It also does not add datum design outputs, CLI
support, location I/O, reader inference, healing, a primary-body rule,

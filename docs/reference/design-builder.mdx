...
This is a deliberately narrow facade for authoring the direct graph, flat local
assemblies, and datum nodes admitted by the four staged v7 geometry evaluators
and the datum resolver. Its implementation lives in
...
imported-body, body-set, part, flat local assembly, and datum records. Its
...
| `assembly(id, build)` | `AssemblyRef` | Flat fixed-placement instances of owned local parts through `StagedLocalAssemblyBuilderV7` |
...
### Flat local assemblies

The staged `assembly(id, build)` method passes one
`StagedLocalAssemblyBuilderV7` to its callback. Each `instance(...)` in this
slice references an owned local part directly. Its stable ID is the complete
one-segment occurrence path for that flat assembly, its placement is an ordered
list of translate, rotate, scale, or mirror operations, and its configuration
selector is exactly one of:

- `inherit`, which evaluates the part in the configuration selected for the
  root assembly;
- `base`, which evaluates the part in the base document context; or
- `named`, which evaluates the part in the referenced document-owned named
  configuration.

Omitted instance options mean an empty placement, `suppressed: false`, and
`{ mode: "inherit" }`.

The root configuration also controls definition-scoped instance suppression
through `configuration.instanceSuppressed(...)`. Passing `false` explicitly
unsuppresses an instance authored with `suppressed: true`; omission retains the
authored value. Suppression is resolved before component support is checked, so
an effectively suppressed nested or external component present in hand-authored
v7 data is inert. An active nested local assembly or external component remains
unsupported and is rejected before resource resolution or kernel work.

Occurrence IDs are unique only within their containing assembly. Repeating one
part in several instances is expected and does not copy its document
definition. Per-occurrence `inherit`, `base`, and named selection can still
produce different effective geometry, material, or density from that shared
definition. Evaluation reuses one part result for equal `(part, effective
configuration)` states within one call, and direct part evaluation may
separately deduplicate leaf acquisition within each configuration batch.
Authoring does not create a persistent or cross-run cache commitment.
...
nested local assemblies, external occurrences, datum-backed sketches,
transforms, Booleans, body-consuming features, per-body materials, or other
general dependency graphs. It also does not add datum design outputs, CLI
...
interference, or exact aggregate STEP/BREP export. Part- and assembly-level
STL/OBJ export is an aggregate tessellation view and is explicitly
approximate/lossy; exact native export and topology remain per-solid or
per-body capabilities. Those omissions are product boundaries, not implicit
fallbacks.

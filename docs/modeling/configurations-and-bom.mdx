...
DAG—including shared Boolean nodes—once per configuration batch when multiple
admitted parts reference it. Different effective configurations resolve and
charge separate operand graphs even when their current numeric results happen
to match. It does not infer geometric equivalence between contexts and
establishes no cross-run cache.
...
configuration)` once across the active tree; dependency counting includes
every Boolean target/tool edge and transform input edge, including repeated
references. Repeated occurrences in the same context share that work,
different contexts are charged independently, and suppressed subtrees
contribute nothing. Active external components and cyclic local-assembly graphs
are rejected before resolver or kernel work. The staged slice does not provide
external-document evaluation, per-body materials, mates, joints, DOF, motion,
contact, interference/collision, assembly topology, aggregate geometric
measurement, or exact assembly STEP/BREP export. Its

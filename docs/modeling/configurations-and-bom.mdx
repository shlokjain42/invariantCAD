...
## Repository-only Document v7 part evaluation

The source tree also contains a staged evaluator for Document v7 outputs that
directly reference a part. This is executable protocol evidence, not a public
authoring workflow: the package-root builder and
`stagedBodySetDesignV7(...)` cannot author a v7 part, and the public document
and evaluator aliases remain v6.
...
material overrides select the effective material identity. An explicit part
density takes precedence over that material's density, just as on the public
v6 evaluator, while a legacy material label remains descriptive and never
supplies inferred density.
...
and per-body materials are outside this staged slice.

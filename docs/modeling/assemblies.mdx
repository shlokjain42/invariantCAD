...
local parts or already-completed local assemblies from the same builder.
Requiring the referenced assembly to be complete makes facade-authored nesting
acyclic by construction. These staged assemblies can be published as direct
outputs and selected by `evaluateLocalAssemblyOutputsV7(...)`. They are not
available from the package root, a package subpath, the public evaluator, or
the CLI.

Evaluation expands active local assemblies iteratively and emits active part
leaves in authored depth-first order. Each leaf part must use geometry already
admitted by the direct staged part evaluator: one supported solid leaf or one
supported body set, including commitment-verified imported leaves. Every leaf
occurrence retains its full root-to-leaf instance path. Its `id` is the final
path segment, while the complete path supplies stable identity independent of
array position or geometry.
...
referenced part or child assembly. Caller parameter overrides specialize every
selected context. Placement operations execute in authored order on each edge,
then nested placements compose parent first so every returned matrix is
root-relative.

Repeated instances of one part retain distinct full paths and quantities while
reusing one evaluated part result when both the part and effective
configuration match. Direct part evaluation may additionally acquire a shared
leaf node once within each configuration batch. Neither optimization creates a
...
existing selected-output, scanned-instance, active-occurrence, placement, part,
solid, material, and resource ceilings. Suppression is resolved before
component admission and prunes an entire nested subtree, so an effectively
suppressed external or otherwise unsupported descendant is inert and charges
no descendant traversal work.

An active external component fails with a structured diagnostic before
resource resolution or geometry-kernel work. Cyclic local-assembly graphs are
not supported: the staged facade cannot construct one, strict document
admission rejects hand-authored dependency cycles, and evaluation also fails
closed if one reaches traversal. External-document evaluation, mates, joints,

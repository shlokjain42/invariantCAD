...
This native boundary remains deliberately unadvertised in the document and kernel protocols. Public draft now depends on the TypeScript lineage reducer plus capability detection through a matched generated JavaScript/WASM pair; loading stock OCCT without the owned probe must not advertise draft.

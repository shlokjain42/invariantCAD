...
- Implement complete backend-owned shape codecs, starting only when they can round-trip every evaluator-observable semantic. In the lockfile-tested Manifold 3.5.1 runtime, public Float32 Mesh reconstruction of a `1 × 2 × 3` box translated by `0.1` on X changes X bounds from `[-0.4, 0.6]` to `[-0.4000000059604645, 0.6000000238418579]` and volume from `6` to `6.000000178813934`; restoring tolerance does not restore the geometry. Then integrate cache read/decode and encode/write into evaluation with cleanup, cancellation, corruption, eviction, and conformance gates. No shipped backend advertises a codec and the evaluator does not consume the protocol yet

...
- Conditional OCCT topology descriptor semantics `@5` for recognized owned facade ABI 0.5+, adding only exact generated edge→face fillet/chamfer class anchors; stock and earlier owned runtimes remain descriptor `@4`, persistent-reference protocol remains v1, and facade ABI remains 0.6
...
- Landed: topology-signature protocol v1 for detached face/edge references, with key-free structured geometry and one-hop adjacency evidence, optional semantic compatibility fingerprints, exact fingerprint gating across stock/older-owned descriptor `@4` and owned ABI 0.5+ descriptor `@5`, partial-history fallback, and fail-closed missing/ambiguous resolution
- Landed: Document v2 topology-reference registries and typed persistent selector atoms, retained through Document v5 with v1–v4 preservation, explicit migration to v5, canonical normalized variants, exact direct-target and fingerprint binding, bounded shared resolution, and fillet/chamfer/shell/draft consumption
...
- Landed: frozen node-kind membership and top-level document-body fields through explicit v1/v2/v3/v4/v5 TypeScript and Zod lists, compile-time tuple/union tripwires, strict runtime inventory tests, and allow-listed migration copying
...
Current topology keys and ABI indices remain evaluation-scoped. Protocol-v1 references persist detached evidence rather than those keys and resolve to a fresh key only for one compatible current candidate. Document v2–v5 can serialize that evidence and selector intent, never the resolved key or native shape. The version axes are independent: Document v5 is the JSON grammar that admits the exact edge-treatment role literals; OCCT descriptor `@4` remains the stock/older-owned compatibility declaration and descriptor `@5` is conditional on recognized owned ABI 0.5+ exact edge-treatment evolution; persistent-reference protocol v1 is the unchanged detached-evidence and matching envelope; facade ABI remains 0.6. Migration to Document v5 never upgrades a stored descriptor fingerprint or rewrites its evidence. This is not a geometric-diff system or cross-run shape cache. Exact ABI 0.4 Boolean, ABI 0.5 fillet/chamfer, and ABI 0.6 shell/offset history prove one evaluation's evolution graph and strengthen semantic-lineage evidence, but do not themselves make a native index persistent. The new treatment roles name classes rather than individual faces, so multiple members fail persistent matching as ambiguous.

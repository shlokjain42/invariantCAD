...
## Staged Document v7 import, transform, product, and assembly status
...
- owner-bound solid DAGs whose leaves are those primitives or imported bodies
  and whose internal nodes are ordered transforms, with typed
  `translate(...)`, `rotate(...)`, `scale(...)`, and `mirror(...)`
  conveniences;
...
- owner-bound body sets whose dense, non-empty ordered memberships reference
  facade-created primitive/import/transform DAG roots;
- owner-bound parts whose geometry is one staged solid DAG root or one admitted
  body set; and
- acyclic nested fixed-placement assemblies whose instances reference owned
  local parts or already-completed local assemblies and select inherited, base,
  or named child configuration contexts, plus outputs that directly target an
  imported body, body set, part, or staged local assembly.
...
- A direct body-set output preserves authored member order, stable member IDs,
  ...
  cylinder, sphere, imported-body leaf, or a bounded transform DAG rooted in
  those leaves. Shared graph nodes are evaluated once without collapsing
  authored memberships.
...
- A direct local-assembly output iteratively expands active local assemblies in
  authored depth-first leaf order. It preserves full root-to-leaf occurrence
  paths, root-relative placements composed parent first, and each part
  occurrence's effective `inherit`, `base`, or named configuration context.
  Every containing assembly's context controls that definition's suppression
  and placement expressions, including explicit `false` unsuppression, before
  component admission. Active local parts may use the same single-solid or
  multibody geometry as the direct part slice, including verified imported
  leaves.

Native primitive/transform body sets can run on either exact or approximate
kernels, and the result records the kernel representation as `mesh`, `brep`,
or `sdf` together with its exactness claim. Stock OCCT constructs exact B-Rep
transforms; Manifold constructs approximate mesh transforms. Evaluation never
falls back from one to the other. An imported member, including an imported
leaf below transforms, is different: the complete body set must use an exact
`brep` kernel with strong single-solid document-body import support. Imported
bytes cross the same verified-resource boundary as a direct imported-body
output. There is no weak `importShape(...)` fallback and no automatic
conversion to a mesh representation.
...
distinct authored memberships; shared transform nodes are memoized the same
way. Aggregate tessellation preserves the authored member sequence, while
aggregate STL and OBJ export are mesh-derived and therefore explicitly
...
invent a compound, or otherwise reinterpret the authored set. Failure releases
every distinct acquired shape exactly once. Success keeps those shapes live
...
transform input edges, authored transform operations, documents, and resolved
resources. The solid-DAG ceilings are `maxSolidGraphNodes`,
`maxSolidDependencyLinks`, and `maxTransformOperations`, each defaulting to
`100,000`; `maxDistinctSolids` separately counts leaves. Those admission limits
do not bound mesh buffers created by later facade calls; applications handling
hostile or very large tessellations still need a disposable worker or process
and an application-owned output ceiling.
...
For a nested local assembly, repeated part instances retain distinct full
occurrence paths and BOM quantities. One evaluated part result is reused for
equal `(part, effective configuration)` contexts within one evaluation call.
Within each effective-configuration batch, direct part evaluation may also
acquire a shared solid DAG once even when multiple admitted parts reference it.
Neither optimization is a cross-run cache or geometry-equivalence claim.
Aggregate assembly mesh, binary/ASCII STL, and OBJ apply each root-relative
occurrence placement and remain approximate/lossy. The contextual BOM keeps
unlike part/configuration states separate, while physical mass applies each
effective density and placement before combining mass, center of mass, and
inertia.
...
and downstream part/resource ceilings. Solid-graph nodes, transform input
edges, and transform operations are counted globally by `(node, effective
configuration)` across active occurrences: repeated occurrences in one context
share the charge, while different contexts are independent. An active external
component is rejected before resolver or kernel work. The facade cannot
construct a cycle because it accepts only already-completed same-builder
assembly handles; strict document admission and evaluation reject hand-authored
recursive graphs.
...
does not author external occurrences, cyclic local-assembly graphs, Boolean
solids, transforms whose input is a body set, part, or assembly, other
body-consuming operations, per-body materials, or general solid graphs beyond
primitive/import/transform DAGs. The facade cannot publish a direct primitive,
generic, or transformed-solid output either; transformed geometry must be
retained by a body set or part. The narrower imported-body handle remains the
only directly publishable solid-like staged reference. The operations perform
no location I/O and admit no healing beyond `none`. A resource's `mediaType` is
committed provenance passed to the resolver; it is not used to choose the
reader or accepted as a substitute for the imported-body node's explicit
`format`. Assembly mates, motion, interference/collision, assembly-wide
topology or geometric measurement, and exact aggregate STEP/BREP remain
unsupported. A versioned format-to-media-type policy is still required before
public v7 promotion.

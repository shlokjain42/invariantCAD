...
The authoring facade and both staged evaluator slices remain repository-only:
none is exported by the package root, available from a package subpath, or
integrated with the public builder or CLI. They do not author or evaluate
parts, assemblies, external occurrences, datums, body-consuming operations, or
general dependency graphs; the facade cannot publish a direct primitive or
generic-solid output either. They also do not define aggregate mass semantics
or a cross-body topology namespace, perform location I/O, or admit healing
...
the imported-body node's explicit `format`. A versioned format-to-media-type
policy is still required before public promotion.

...
## Staged Document v7 import, body-set, part, and local-assembly status
...
- typed length and mass-density parameters, named parameter configurations,
  document-owned materials, typed part-material substitutions, and
  assembly-definition instance-suppression overrides;
...
- owner-bound body sets whose dense, non-empty ordered memberships directly
  reference facade-created primitives or imported bodies;
- owner-bound parts whose geometry is one direct leaf or one admitted body set;
  and
...
- A direct part output preserves part metadata and effective material/density
  ...
  branch. Its geometry must be one supported direct solid leaf or one body set
  admitted by the preceding slice. The staged facade authors exactly this
  direct part boundary through `material(...)`, `part(...)`, configuration
...
acquire a shared leaf node once even when multiple admitted parts reference it.
...
and downstream part/resource ceilings. An active external component is
rejected before resolver or kernel work. The facade cannot construct a cycle
because it accepts only already-completed same-builder assembly handles; strict
document admission and evaluation reject hand-authored recursive graphs.
...
does not author external occurrences, cyclic local-assembly graphs, solid
transform nodes, Booleans, body-consuming operations, per-body materials, or
general dependency graphs, and the staged geometry evaluators do not evaluate
those structures. The facade cannot publish a direct primitive or generic-solid
output either. The operations perform no location I/O and admit no healing
beyond `none`. A resource's `mediaType` is committed provenance passed to the
resolver; it is not used to choose the reader or accepted as a substitute for
the imported-body node's explicit `format`. Assembly mates, motion,
interference/collision, assembly-wide topology or geometric measurement, and
exact aggregate STEP/BREP remain unsupported. A versioned
format-to-media-type policy is still required before public v7 promotion.

...
The authoring facade, all three staged geometry-evaluator slices, and the
...
does not author assemblies, external occurrences, transforms, Booleans,
body-consuming operations, per-body materials, or general dependency graphs,
and the staged geometry evaluators do not evaluate those structures. The
facade cannot publish a direct primitive or generic-solid output either. The

...
- typed length and mass-density parameters, named parameter configurations,
  document-owned materials, and typed part-material substitutions;
...
- owner-bound parts whose geometry is one direct leaf or one admitted body set,
  plus outputs that directly target an imported body, body set, or part.
...
evaluators perform file, URL, or package I/O. Building validates namespaces,
commitments, owner identity, material and configuration references, dense
collections, unique member IDs, and caller-selected `DesignDocumentLimits`
before returning detached, deeply frozen v7 data. Immediate authoring ceilings
default to
...
Parameter, configuration, material, part, resource, import, unit-policy, and
body-member options must be plain own-data records. Accessor-backed and unknown
...
not author assemblies, external occurrences, datums, transforms, Booleans,
body-consuming operations, per-body materials, or general dependency graphs,
and the staged evaluators do not evaluate those structures. The facade cannot
publish a direct primitive or generic-solid output either. The operations
perform no location I/O and admit no healing beyond `none`. A resource's
`mediaType` is committed provenance passed to the resolver; it is not used to

...
- typed length and mass-density parameters, named parameter configurations,
  document-owned materials, typed part-material substitutions, and root
  assembly instance-suppression overrides;
...
- owner-bound parts whose geometry is one direct leaf or one admitted body set;
  and
- flat fixed-placement assemblies whose instances directly reference owned
  local parts and select inherited, base, or named child configuration
  contexts, plus outputs that directly target an imported body, body set, part,
  or staged local assembly.
...
- A direct flat local-assembly output preserves active occurrence order, stable
  one-segment IDs, resolved placements, and each part occurrence's effective
  `inherit`, `base`, or named configuration context. Root configuration
  suppression, including explicit `false` unsuppression, is resolved before
...
For a flat local assembly, repeated part instances retain distinct occurrence
IDs and BOM quantities. One evaluated part result is reused for equal
`(part, effective configuration)` contexts within one evaluation call. Within
each effective-configuration batch, direct part evaluation may also acquire a
shared leaf node once even when multiple admitted parts reference it. Neither
optimization is a cross-run cache or geometry-equivalence claim. Aggregate
assembly mesh, binary/ASCII STL, and OBJ apply each occurrence placement and
remain approximate/lossy. The contextual BOM keeps unlike part/configuration
states separate, while physical mass applies each effective density and affine
placement before combining mass, center of mass, and inertia. An effectively
suppressed unsupported component is inert. An active nested local assembly or
external component is rejected before resolver or kernel work.
...
does not author nested local assemblies, external occurrences, transforms,
Booleans, body-consuming operations, per-body materials, or general dependency
graphs, and the staged geometry evaluators do not evaluate those structures.
The facade cannot publish a direct primitive or generic-solid output either.
...
node's explicit `format`. Assembly mates, interference/collision, assembly-wide

...
## Staged Document v7 import, body-set, and part status
...
- length parameters and named parameter configurations, plus direct boxes,
  cylinders, and spheres, by privately composing the existing v6 builder;
...
- owner-bound body sets whose dense, non-empty ordered memberships directly
  reference facade-created primitives or imported bodies; and
- outputs that directly target an imported body or body set.
...
resource locations are resolver hints only. Neither the facade nor either
evaluator performs file, URL, or package I/O. Building validates namespaces,
commitments, owner identity, configuration references, dense collections,
unique member IDs, and caller-selected `DesignDocumentLimits` before returning
detached, deeply frozen v7 data. Immediate authoring ceilings default to
...
Resource, import, unit-policy, and body-member records must be plain own-data
records. Accessor-backed and unknown fields are rejected rather than invoked
or silently interpreted.

The repository has three source-only staged evaluator slices:
...
- A direct part output preserves part metadata and effective material/density
  ...
  admitted by the preceding slice. This evaluator does not imply that the
  staged facade can author parts.
...
legacy material label is never used as a catalogue lookup. The one-row part
BOM and `physicalMassProperties()` use one uniform effective density across
all memberships. Mass is additive over independent bodies: repeated aliases
and spatial overlaps each contribute once per authored membership rather than
...
The authoring facade and all three staged evaluator slices remain
repository-only: none is exported by the package root, available from a
package subpath, or integrated with the public builder or CLI. The facade does
not author parts, and the evaluators do not evaluate assemblies, external
occurrences, datums, body-consuming operations, or general dependency graphs.
...
not used to choose the reader or accepted as a substitute for the
imported-body node's explicit `format`. A versioned format-to-media-type policy
is still required before public promotion.

...
An active external component is rejected before resolver or kernel work. The
facade cannot construct a cycle because it accepts only already-completed
same-builder assembly handles; strict document admission and evaluation reject
hand-authored recursive graphs.
...
does not author external occurrences, cyclic local-assembly graphs, Boolean or
transform inputs that are body sets, parts, or assemblies, other
body-consuming operations, per-body materials, or general solid graphs beyond
primitive/import/Boolean/transform DAGs. The facade cannot publish a direct

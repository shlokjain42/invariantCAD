...
- Made exact indexed Boolean evolution an optional evaluator capability: ABI 0.4 uses the exact transaction, malformed advertised metadata fails closed, and stock OCCT, owned ABI 0.2/0.3, and Manifold continue through their partial-history Boolean implementations. Empty results retain the existing `EMPTY_RESULT` / `allowEmpty` behavior, topology keys remain evaluation-scoped, and fillet/chamfer/shell/offset evolution remains partial.
- Advanced the local package-neutral OCCT facade ABI and bundle to 0.4.0, adding `0004-exact-boolean-history.patch` to the reproducible source/relink series and extending native, public-adapter, and packed-consumer coverage for the exact Boolean surface while retaining ABI 0.3's controlled PipeShell contract.
